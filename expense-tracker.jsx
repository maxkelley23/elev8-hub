...
export default function ExpenseTracker() {
  ...
  const annualExpenses = expenses
    .filter(e => e.cycle === 'Annual')
    .reduce((sum, e) => sum + e.amount, 0);

  const oneTimeExpenses = expenses
    .filter(e => e.cycle === 'One-time')
    .reduce((sum, e) => sum + e.amount, 0);
  ...
  const addRevenue = () => {
    if (newRevenue.source && newRevenue.amount) {
      const maxId = revenue.length > 0 ? Math.max(...revenue.map(r => r.id)) : 0;
      setRevenue([...revenue, {
        id: maxId + 1,
  ...
  const saveExpenseEdit = () => {
    setExpenses(expenses.map(e =>
      e.id === editingExpenseId
        ? { ...e, ...editExpense, amount: parseFloat(editExpense.amount) }
        : e
    ));
  ...
  const saveRevenueEdit = () => {
    setRevenue(revenue.map(r =>
      r.id === editingRevenueId
        ? { ...r, ...editRevenue, amount: parseFloat(editRevenue.amount) }
        : r
    ));
